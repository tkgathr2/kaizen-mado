...
//   /api/line             … LINE webhook（x-line-signatureで別途保護済。GO検知の入口）
//   /_next/* favicon 等    … Next静的アセット
// 保護対象：ページ(/) ・ /api/submit ・ /api/chat
//
// 重要：execute/line/admin は Cronsecret/署名検証で独自認証しており Cookieセッションを持たない。
// これらを matcher から除外しないと、認証ON時にサインインへリダイレクトされ、
// 改修パイプライン（execute）とGO検知（line/admin）が全停止する。
...
export const config = {
  ...
  matcher: [
    "/((?!api/auth|api/process|api/learn|api/execute|api/line|api/admin|widget.js|kaizen-kun.png|_next/static|_next/image|favicon.ico).*)",
  ],
};
