...
describe("resolveReporterDisplay（起票者の表示名解決・2026-07-08 社長要望対応）", () => {
  let originalFetch: typeof global.fetch;
  const ENV = ["SLACK_BOT_TOKEN", "KAIZEN_SLACK_USER_NAMES"] as const;
  const saved: Record<string, string | undefined> = {};
  ...
  beforeEach(() => {
    ...
    // 既知ユーザー対応表（本物の氏名はenvで運用。テストはダミー名）
  ...
  it("Slackメンション形式はトークン未設定でも既知ユーザー対応表で解決する", async () => {
    const mockFetch = vi.fn();
    global.fetch = mockFetch as unknown as typeof global.fetch;
  ...
  it("トークン未設定かつ対応表にも無いIDは元の文字列にフォールバック", async () => {
    const mockFetch = vi.fn();
    global.fetch = mockFetch as unknown as typeof global.fetch;
    const r = await resolveReporterDisplay("Slack:<@UZZ9NOTINMAP>");
    expect(r).toBe("Slack:<@UZZ9NOTINMAP>");
    expect(mockFetch).not.toHaveBeenCalled();
  });

  it("Notion往復のバックスラッシュ付き（Slack:\\<@U…\\>）も解決できる", async () => {
    const mockFetch = vi.fn();
    global.fetch = mockFetch as unknown as typeof global.fetch;
    const r = await resolveReporterDisplay("Slack:\\<@U0AR8F63YBA\\>");
    expect(r).toBe("西村克人");
  ...
  it("users.info が失敗（users:read不足等）しても既知ユーザー対応表で解決する", async () => {
    process.env.SLACK_BOT_TOKEN = "xoxb-test";
    const mockFetch = vi.fn().mockResolvedValue({
  ...
  it("users.info 失敗かつ対応表にも無いIDは元の文字列にフォールバック（通知は止めない）", async () => {
    process.env.SLACK_BOT_TOKEN = "xoxb-test";
    const mockFetch = vi.fn().mockResolvedValue({
...
describe("readableReporter（表示直前の最終ガード・生IDを社長に見せない）", () => {
  ...
  it("既知ユーザーIDは名前になる", () => {
    expect(readableReporter("Slack:<@U0AR8F63YBA>")).toBe("西村克人");
  });

  it("未知のIDは生IDを出さず「Slackの方（お名前未登録）」", () => {
    expect(readableReporter("Slack:<@UZZ9NOTINMAP>")).toBe("Slackの方（お名前未登録）");
  });

  it("バックスラッシュ付きも同様に処理する", () => {
    expect(readableReporter("Slack:\\<@U0AR8F63YBA\\>")).toBe("西村克人");
  });

  it("通常の氏名・空はそのまま／不明", () => {
    expect(readableReporter("現場フォーム")).toBe("現場フォーム");
    expect(readableReporter("")).toBe("不明");
  });
});
