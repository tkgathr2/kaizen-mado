...
describe("shouldProtectPath（optional auth・管理ページだけ強制保護）", () => {
  ...
  it("管理ページが読むデータAPI(/api/board・/api/stats)も保護する（true＝認可漏れ防止）", () => {
    expect(shouldProtectPath("/api/board")).toBe(true);
    expect(shouldProtectPath("/api/board/")).toBe(true);
    expect(shouldProtectPath("/api/stats")).toBe(true);
    expect(shouldProtectPath("/api/stats/summary")).toBe(true);
  });
  ...
  it("保護プレフィックスに前方一致するだけの別パスは保護しない（誤爆防止）", () => {
    expect(shouldProtectPath("/boardroom")).toBe(false);
    expect(shouldProtectPath("/dashboards")).toBe(false);
    expect(shouldProtectPath("/api/boardroom")).toBe(false);
    expect(shouldProtectPath("/api/statsx")).toBe(false);
    // 起票導線は公開のまま（保護プレフィックスに含めない）
    expect(shouldProtectPath("/api/submit")).toBe(false);
    expect(shouldProtectPath("/api/chat")).toBe(false);
  });
