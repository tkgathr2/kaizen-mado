...
/** 【2026-08-12 社長指示：process では未使用になった】
 * 「重要度・危険度に関係なく全チケットが必ず社長のLINE確認を経由する」運用へ変更したため、
 * app/api/process/route.ts のオートパイロット分岐（安全案件を社長に聞かず自動着手）を廃止した。
 * 同時に preGate も process からは呼ばれなくなっている（本番コードの呼び出し元は現在ゼロ）。
 * 将来の再有効化と、判定ロジックの回帰テスト（lib/__tests__/gate.test.ts）を残すため、
 * 関数自体は削除せずここに残す。分岐の復活は社長の明示指示があったときだけ。
 *
 * 真田自走（オートパイロット）が有効か。
